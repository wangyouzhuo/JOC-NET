...
class ACNet(object):
        ...
        with tf.device(device):
            ...
            if scope == 'Weight_Store':
                ...
                with tf.variable_scope(scope):
                    ...
                    self.global_a_prob = self._build_global_net(scope)
    ...
    def _prepare_global_params(self, scope):
        with tf.variable_scope(scope):
            ...
            # fusion
            self.w_fusion = generate_fc_weight(shape=[2048*2, 2048], name='global_w_f')
            self.b_fusion = generate_fc_bias(shape=[2048]        , name='global_b_f')
            # scene
            self.w_scene  = generate_fc_weight(shape=[2048, 512]  , name='global_w_s')
            self.b_scene  = generate_fc_bias(shape=[512]         , name='global_b_s')
            # actor
            self.w_actor  = generate_fc_weight(shape=[512, 4] , name='global_w_a')
    ...
    def _build_global_net(self, scope):
        with tf.variable_scope(scope):
            ...
            scene_layer = tf.nn.elu(tf.matmul(fusion_layer, self.w_scene) + self.b_scene)

            # scene_layer --> prob
            self.global_logits = tf.matmul(scene_layer, self.w_actor) + self.b_actor
