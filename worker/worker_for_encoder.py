...
class Encoder_Network(object):
    ...
    def __init__(self,sess,scope):
        ...
        with tf.name_scope(scope):
            ...
            self.action_predict_loss = -tf.reduce_mean(tf.one_hot(self.action,4,dtype=tf.float32)*tf.log(tf.clip_by_value(self.action_predicted,1e-10,1.0)))
            ...
            self.state_feature_predict = tf.nn.tanh(tf.matmul(self.cur_action_concat, self.weight_p_n_s_1) + self.bias_p_n_s_1)
            ...
            self._build_and_prepare_auto_encoder(input=self.current_state_feature)
    ...
    def _prepare_weight(self):
        ...
        self.fc_weight    = generate_fc_weight(shape=[2240,2048]    ,name='fc_weight_encode')
        self.fc_bias      = generate_fc_weight(shape=[2048]         ,name='fc_bias_encode')
        self.encode_params = [
        ...
        # decode weight
        self.fc_for_decoder = generate_fc_weight(shape=[2048,2240]   ,name='fc_decoder_weight')
        self.bias_for_decoder    = generate_fc_bias(shape=[2240]     ,name='bias_decoder_weight')
        ...
        self.weight_p_a_1    = generate_fc_weight(shape=[2048*2,2048],name='p_action_weight_1')
        self.bias_p_a_1      = generate_fc_bias(shape=[2048]         ,name='p_action_bias_1')
        self.weight_p_a_2    = generate_fc_weight(shape=[2048,4]     ,name='p_action_weight_2')
    ...
    def _build_encode_net(self,input_image):
        ...
        print("self.conv4_bias:",self.conv4_weight.shape)
        print("conv4 : ",conv4)
        print("after flatten : ",flatten_feature)
        state_feature = tf.nn.sigmoid(tf.matmul(flatten_feature, self.fc_weight) + self.fc_bias) # 4096
        # return flatten_feature,state_feature,feature_map
        return state_feature

    def _build_decode_net(self,input_feature):
        # 4096 --> 4160
        input_feature = (tf.matmul(input_feature, self.fc_for_decoder) + self.bias_for_decoder)

        input_feature = inverse_flatten(input_feature,(-1,5,7,64))
        print("input_feature ：",input_feature)
        resize5 = tf.image.resize_images(input_feature,size=(13,17),method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        ...
        resize6 = tf.image.resize_images(conv5,size=(25,34),method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        ...
        resize7 = tf.image.resize_images(conv6,size=(75,100),method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        ...
        resize8 = tf.image.resize_images(conv7,size=(300,400),method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        conv8 = tf.nn.conv2d(resize8, self.conv8_weight,strides=[1,1,1,1], padding='SAME')

        decoded_image = tf.nn.sigmoid(tf.nn.bias_add(conv8, self.conv8_bias))
